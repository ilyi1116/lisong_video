...
class Downloader {
    ...
    async _downloadM3U8() {
        ...
        this.movie.type = MOVIE_TYPE.M3U8;
        if (this.movie.staticUrls && this.movie.staticUrls.length > 0) {
            for (let url of this.movie.staticUrls.slice()) {
                m3u8 = await this._safeDownloadFile(url, null, this._options.downloadMaxRetry);
                if (m3u8.code === 0) {
                    m3u8.url = url;
                    break;
                }
    ...
    async _safeDownloadFile(url, path: string = null, maxRetry: number = 10, onProgress: Function = null) {
        let _download = async (resolve, reject) => {
            try {
                let result = await this._downloadFile(url, path, onProgress);
                console.log('_downloadFile:', result);
                if (result.code === 0) {
                    if (resolve)
    ...
    async _downloadFile(url, path: string = null, onProgress: Function = null) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this._downloadTask
                ...
                .then(async (resp) => {
                    let info = resp.info();
                    let data = {
                        code: info.status === 200 ? 0 : info.status,
                    };
                    if (info.status === 200) {
                        data['path'] = resp.path();
                        if (path === null) {
     ...
     */
    async start() {
        //console.log('Downloader.start:', this.isStartDownload);
        if (this.isStartDownload) {
            return;
        }
        ...
        let _download = () => {
            ...
            return new Promise(async (resolve, reject) => {
                ...
                //console.log('Downloader.start', result);
                if (result.code === 0) {
                    this._onDone();
     ...
     */
    async stop() {
        //console.log('Downloader.stop:', this.isStartDownload);
        this.isStartDownload = false;
        this._status = DOWNLOAD_STATUS.Waiting;
        if (this._downloadTask) {
            this._downloadTask.cancel((err) => {
                console.log('Cancel Download:', err);
    ...
    getMoviePath() {
        ...
        // 播放源文件地址
        const playFile = savePath + this.movie.mark + '.' + (this.movie.type || MOVIE_TYPE.M3U8);
        // 播放源url
        const playSource = staticServerHost + this.movie.mark + '/' + this.movie.mark + '.' + (this.movie.type || MOVIE_TYPE.M3U8);
...
class DownloadManager {
    ...
    async _initialize() {
        ...
        let self = this;
        let fileName = CACHE_ROOT_DIR + '.cache.json';
    ...
    async _initializeDir() {
        if (!(await RNFetchBlob.fs.isDir(CACHE_ROOT_DIR))) {
            await RNFetchBlob.fs.mkdir(CACHE_ROOT_DIR);
        }
    }
    ...
    _save() {
        ...
        RNFetchBlob.fs.writeFile(CACHE_ROOT_DIR + '.cache.json', data, 'utf8');
    }
     ...
     * @private
     */
    _addData(status, movie:Video, stream: string) {
        ...
        let staticUrls = null;
        // 获取静态地址
        if (!util.isEmptyObject(movie.hotlinkContent) && !util.isEmptyObject(movie.hotlinkContent.urls)) {
            staticUrls = movie.hotlinkContent.urls[stream];
        }
        ...
        let _download = () => {
            return new Promise(async (resolve, reject) => {
                ...
                while (true) {
                    ...
                    //console.log('CacheManager._download', result);
                    if (result.code === 0) {
     ...
     * @param stream 清晰度
     */
    async add(movie:Video, stream: string, autoStart: boolean = true) {

        let download = async (resolve, reject) => {
            // 写入缓存中
            this._addData(STATUS.Caching, movie, stream);
