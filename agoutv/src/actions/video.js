...
import {callApi, safeCallApi} from "../middlewares/api";
...
export const loadVideoDetail = (code:string) => (dispatch: Dispatch, getState: GetState) => {
    ...
    if (video && (Date.now() - video.timeUpdated) < 1800000) {
        console.log('loadVideoDetail from cache:', video.timeUpdated);
        return null;
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
const _playVideoAction = (movieId: number, code: string, episode : number, seriaId:number): ThunkAction => ({
    params: {
        episode:episode,
        stateKeys: ['video', code]//, 'playLink', episode.toString()],
    },
...
export const playVideo = (movieId: number, code: string, episode : number, seriaId:number) => (dispatch: Dispatch, getState: GetState) => {
    return dispatch(_playVideoAction(movieId, code, episode, seriaId));
};
